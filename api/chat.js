...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const bodyAdd = `question=${encodeURIComponent(prompt)}&bfp_hash=277174405&bfp_data=&ip=103.30.11.154&client_token=${randomClientToken}`;
    ...
    const bodyCheck = `question_hash=${questionHash}&bfp_hash=277174405&bfp_data=&ip=103.30.11.154&lang=en`;
    ...
    const finishReason = dataCheck.data.finish_reason;

    // Hanya tampilkan answer_text jika finish_reason adalah "stop"
    if (finishReason === 'stop') {
      res.status(200).send(answerText);
    } else {
      res.status(200).send('No answer available');
    }
  } catch (error) {
