    ...
    https.get(url, (response) => {
        ...
        response.on('end', () => {
            ...
            episodeElements.forEach(episode => {
                ...
                // Menambahkan `current` dan `durasi` ke dalam objek episode
                episodes.push({
                    episode: episode.textContent.trim(),
                    slug: slug,
                    current: "0",  // Misalnya, episode ini adalah episode pertama atau durasi saat ini
                    durasi: "0"  // Durasi total episode dalam menit atau detik, dalam bentuk string
                });
            });
